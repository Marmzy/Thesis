...
class blast_util:
	...
	def batch_bl2(self, program, pairs, fasta1, fasta2, bdir, outflag=0,
		...
		for i in gpairs:
			...
			# call bl2seq and parse the output
			os.system("%s/bl2seq -p %s -i %sTMP.FA1 -j %sTMP.FA2 -F F -e 100 -o %sTMP.OUT -W %i"%\
					   (bdir,program,wdir,wdir,wdir,W))
	...
	def parse_tfasty(self,f,call=0):
		...
		while inl != "":
			...
			# tag1 found already
				...
				#print(info[8], foundTag2, inl[:-1], seq)
				if inl[:-1] != "":
					if inl[0] == ">" and inl[1] != "-":
	...
	def help(self):
		...
		print "        OPTIONAL: j,bdir,d,wdir,W,top"
		print "     batch_sw - do batch tfasty with smith-waterman algorithm"
		print "        NEED: p,g,i, OPT: j, bdir, d"
		...
		print " -stype sequence type, pep (default) or nt"
...
if __name__ == '__main__':
	...
	if f == "blast_window":
		...
		if "" in [p,g,fasta]:
			...
			util.help()
		util.batch_bl2(p,g,fasta,fasta2,bdir,d,wdir,W,top)
	elif f == "batch_sw":
		if "" in [p,g,fasta]:
			print "\nRequires program, gene pairs, fasta"
			util.help()
