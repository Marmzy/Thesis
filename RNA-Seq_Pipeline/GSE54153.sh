...
#This script was made to download the GEO accession GSE54153 fastq files from EBI.
...
for i in {1121292..1121303}
do
	num="${i: -1}"
        wget ftp://ftp.sra.ebi.ac.uk/vol1/fastq/SRR112/00$num/SRR$i/SRR$i.fastq.gz
done
...
do	
	...
	base=$(grep -E 'Per base sequence quality' /group/biocomp/users/cadav/Thesis/Transcriptome/RNA-Seq/GSE54153/TrimmedQC/SRR$num.trimmed_fastqc/fastqc_data.txt)
	result=$(echo $base | tail -c 5)

        if [ $result = 'fail' ]
        then
		java -jar /software/shared/apps/x86_64/trimmomatic/0.36/trimmomatic-0.36.jar SE -phred33 /group/biocomp/users/cadav/Thesis/Transcriptome/RNA-Seq/GSE54153/SRR$num.trimmed.fastq.gz /group/biocomp/users/cadav/Thesis/Transcriptome/RNA-Seq/GSE54153/SRR$num.quality.fastq.gz SLIDINGWINDOW:5:28 MAXINFO:90:0.1 MINLEN:35
